...
- [x] **2.3 In-process impl** (`fixo-brain.ts`) — DONE:
  ...
  - [x] `recordEstimate` + assembly — the OLP pricing engine is ALREADY shared (`skills/estimate/pricing.ts`, used by HMLS `create_order` + Fixo), so estimates are NOT re-priced through the brain (rerouting the untested central order tool = risk, no functional gain). Instead the brain RECORDS the priced estimate: `recordEstimate` attaches `predicted_estimate` to the prediction; `create_order` calls it best-effort after diagnose. Contract revised (`estimate` → `recordEstimate`); full `brain: BrainService = { diagnose, recordEstimate, recordOutcome }` assembled.
